...
public record UpdateCatalogueItemDto(
    ...
    public static CatalogueItem toModel(UpdateCatalogueItemDto dto){
        return CatalogueItemBuilder.builder().id(dto.id()).model(dto.model()).level(dto.level())
            .type(dto.type()).description(dto.description()).manufacturer(dto.manufacturer()).build();
    }
}
